...
?>
	...
	<table class="table table-bordered table-striped table-sm" width="100%" >
        <thead >
			...
			<td align='center' width="100px;">Coupons <br><br> <span style="font-size: 10px;">Break.,Lunch,Dinner</span></td>
		...
		foreach($emp as $e)
		{
            ...
            <tr >
				...
				<?php 
					if($canteenShow == 'Yes'){
						$from_to_date = date("Y-m-01");
						$coupons =  $this->Hrmodel->get_total_amt_of_emp($from_to_date,$from_to_date,$pay_code);
						if(!empty($coupons)){
							echo $coupons[0]['br_coupon'].', '.$coupons[0]['lunch_coupon'].', '.$coupons[0]['dinner_coupon'];
							echo "<br>";
				...
				<td align='center'>
					<?php 
						...
						if(!empty($coupons)) echo $coupons[0]['br_coupon'].', '.$coupons[0]['lunch_coupon'].', '.$coupons[0]['dinner_coupon'];
					?>
				</td>
				<td align='center'>
					<?php 
						...
						//echo "<hr>";
						//if(isset($out[0]['lost_canteen_payable'])){ echo $lost_canteen_payable = $out[0]['lost_canteen_payable'];}
   ...
   <div class="print_hide">
		<p style="font-size: 16px;" align="right">Canteen Coupon total amount: <?php if(!empty($coupon_amt))echo array_sum($coupon_amt);?> Rs.</p>
		<p style="font-size: 16px;" align="right">Advance total amount: <?php if(!empty($advance_amt))echo array_sum($advance_amt);?> Rs.</p>
  </div>
