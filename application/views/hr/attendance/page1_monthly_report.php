...
if(isset($_REQUEST['canteenShow'])){$canteenShow = $_REQUEST['canteenShow'];}else{$canteenShow = "NO";}
	...
	$fdate = $this->Base->change_date_ymd("$year_search-$month_search-01");
	$tdate = $this->Base->get_last_full_date_of_month_ymd($month_search,$year_search);
	...
	<head>
		...
		<style>
			@media print {
				...
				.no-print, .sidebar, .navbar, .footer, .printDisplay {
...
if(!empty($emp))
{
	...
	//seting day value 0 or 1
		...
		function get_color($data,$no,$no_of_restday_array)
		{
			...
			elseif($data == "HA"){echo "style='color:orange; font-weight:bold; background-color:$bgcolor; '";}
			elseif($data == "HL"){echo "style='color:pink; font-weight:bold; background-color:$bgcolor; '";}
			elseif($data == "A"){echo "style='color:red; font-weight:bold; background-color:$bgcolor; '";}
			elseif($data == "H"){echo "style='color:yellow; font-weight:bold; background-color:$bgcolor; '";}
			elseif($data == "L"){echo "style='color:red; font-weight:bold; background-color:$bgcolor; '";}
...
?>
	...
	<table class="table table-bordered table-striped table-sm" width="100%" >
        <thead >
        	...
        	<th>E. Code</th>
			<th>Dept.</th>
			<th class="printDisplay">Designation</th>
			<th class="printDisplay">Rest Day</th>
			...
			<?php 
				if($canteenShow == 'Yes'){
					?>
					<td align='center' width="100px;" class="printDisplay">Coupons <br><br> <span style="font-size: 10px;">Break.,Lunch,Dinner</span></td>
					<?php
				}//$canteenShow
			?>
			
			<td align='center' width="70px;">Canteen (Rs.)</td>
			<td align='center' width="70px;">Advance (Rs.)</td>
        </tr>
        </thead> 
        <tbody id="tablebody">
		<?php 
		...
		foreach($emp as $e)
		{
			...
			$pay_code2=$e['id'];
			$query=" 
						SELECT E.father_name,E.epf_code,E.esi_code,E.bank_account_no,E.restday,D.name as dname, R.name as rname,E.doj
						FROM employee as E 
			...
			$newStatus = "";
			if (!empty($out_emp[0]['doj']) && $out_emp[0]['doj'] != '0000-00-00') {
				$doj = $out_emp[0]['doj'];

				$dojYear = date('Y', strtotime($doj));
				$dojMonth = date('n', strtotime($doj)); // 'n' gives month without leading zero

				if ((int)$dojYear === (int)$year && (int)$dojMonth === (int)$month) {
					$newStatus = 'new';
				}
			}
			...
			if(isset($out[0]['total_ot'])){ $total_ot = round($out[0]['total_ot']);}else{$total_ot = '';}
            ...
            <tr >
            	...
            	<td  width="250px;">
					 <a  target="_blank"  href="<?php echo base_url().'index.php/Welcome/home?';?>Hr/profile/<?php echo $pay_code;?>">
						<?php echo $fullname;?>
					</a>	
					<span class="badge bg-info" style="color:white"><?php echo $newStatus;?></span>
				</td>
            	<td  width="100px;">
				...
				<?php if($attendance_time == 'Yes')$out9=$this->Hrmodel->print_attendance_date_time($pay_code,'07',$month,$year); ?> <?php echo $d7;?><br><span style="background-color:<?php if($o7>4){echo "yellow";}?>;"><?php echo $o7;?></span><?php 
					if($canteenShow == 'Yes'){
						$from_to_date = date("$year-$month-07");
						$coupons =  $this->Hrmodel->get_total_amt_of_emp($from_to_date,$from_to_date,$pay_code);
						if(!empty($coupons)){
							echo "<br><br>".$coupons[0]['br_coupon'].'<br>'.$coupons[0]['lunch_coupon'].'<br>'.$coupons[0]['dinner_coupon'];
							echo "<br>";
				...
				<td onclick="fun_get_popup_data(this.id)"id="<?php echo $pay_code2.'~'.$pay_code.'~'.'23'.'~'.$d23.'~'.$o23.'~'.$fullname.'~'.$restDay;?>" class="<?php echo "d23-".$pay_code2;?>" title="23" align='center'  <?php echo get_color($d23,23,$no_of_restday_array)?>> <?php if($attendance_time == 'Yes')$out9=$this->Hrmodel->print_attendance_date_time($pay_code,'23',$month,$year); ?>  <?php echo $d23;?><br><span style="background-color:<?php if($o23>4){echo "yellow";}?>;"><?php echo $o23;?></span><?php 
					if($canteenShow == 'Yes'){
						$from_to_date = date("$year-$month-23");
						$coupons =  $this->Hrmodel->get_total_amt_of_emp($from_to_date,$from_to_date,$pay_code);
						if(!empty($coupons)){
							echo "<br><br>".$coupons[0]['br_coupon'].'<br>'.$coupons[0]['lunch_coupon'].'<br>'.$coupons[0]['dinner_coupon'];
							echo "<br>";
				...
				<?php 
					$coupons =  $this->Hrmodel->get_total_amt_of_emp($fdate,$tdate,$pay_code);
					if($canteenShow == 'Yes'){
						?>
						<td align='center' class="printDisplay">
							<?php if(!empty($coupons)) echo "<br><br>".$coupons[0]['br_coupon'].'<br>'.$coupons[0]['lunch_coupon'].'<br>'.$coupons[0]['dinner_coupon'];?>
						</td>
						<?php
					}//$canteenShow
				?>
				...
				<td align='center'>
					<?php 
						if(!empty($coupons)) echo $coupon_amt[] = round($coupons[0]['total_amt']);
					?>
				</td>
				<td align='center'>
					<?php 
						echo $advance_amt[] =  $this->Hrmodel->get_total_advance_amt_of_emp($fdate,$tdate,$pay_code);
						echo "<br>";
						$fine = $this->Hrmodel->get_total_fine_amt_of_emp($fdate,$tdate,$pay_code);
						if(!empty($fine))echo "<br> Fine: ";echo $fine_amt[] = $fine;
					?>
				</td>
			</tr>
			<?php
   ...
   <div class="print_hide">
		<p style="font-size: 16px;" align="right">Total Canteen Coupon amount: <?php if(!empty($coupon_amt))echo array_sum($coupon_amt);?> Rs.</p>
		<p style="font-size: 16px;" align="right">Total Advance amount: <?php if(!empty($advance_amt))echo array_sum($advance_amt);?> Rs.</p>
		<p style="font-size: 16px;" align="right">Total Fine amount: <?php if(!empty($fine_amt))echo array_sum($fine_amt);?> Rs.</p>
  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body" style="font-size:18px">
		...
		Half Day : <input type="radio"  name="now_attendance" value="HA"><br>
		...
		Other Leave : <input type="radio"  name="now_attendance" value="OL"><br>
		Half Day + Half Leave : <input type="radio"  name="now_attendance" value="HL"><br>
		<br>
		Over Time : <input type="text" id="model_ot" class="form-control"  />
		<br>
