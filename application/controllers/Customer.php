	...
	public function save()
	{
		...
		if(empty($_REQUEST['id']) and !empty($_REQUEST['gst']))
		{
			...
			else 
			{
				...
				if ($cat_id > 0) {
					// Create corresponding ledger in acc_ledgers under group 11 (Sundry Debtors)
					$ledger_data = array(
						'name' => NULL,
						'group_id' => 11,
						...
						'opening_type' => 'Dr',
						'customer_id' => $cat_id,
						'description' => 'Auto-created customer account'
					);
		...
		elseif(!empty($_REQUEST['id']) and !empty($_REQUEST['gst']))
		{
			...
			else
			{
				...
				// Update or create corresponding ledger in acc_ledgers to keep in sync
				$ledger_exists = $this->db->get_where('acc_ledgers', array('customer_id' => $id))->num_rows();
				if ($ledger_exists > 0) {
					// No name update needed since customer name is only in customer table and ledger name is NULL
				} else {
					$ledger_data = array(
						'name' => NULL,
						'group_id' => 11,
