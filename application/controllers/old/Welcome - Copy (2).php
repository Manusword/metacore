	...
	*/
	...
	public function emp_attend_api()
	{
		...
		if (!empty($_REQUEST['bio_code'])) {
			...
			$row = $this->db->query($query, [$bio_code])->row_array();
	...
	{
		$query = "SELECT id FROM employee WHERE emp_code='$emp_code' and active='Active' ";
	...
	public function entry_attendance_via_machine($flag, $data_list)
	{
		...
		$emp = $this->Mymodel->query1("SELECT id, company_role, shift_status, current_shift, get_overtime,restday FROM employee WHERE emp_code='$emp_code'");
		if (empty($emp)) return;
