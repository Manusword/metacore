...
class Dispatch extends CI_Controller {
	...
	public function dispatch_get_product2()
	{
		if(isset($_REQUEST['customer_id']))
		{
			...
			//$fromDate = date('2023-04-01');
			//$toDate = date('2024-04-30');
		    $toDate = date('Y-m-t');
	...
	public function schedule_list()
	{
		...
		if(isset($_REQUEST['search1']))
		{
			...
			$where.=" GROUP BY A.schedule_id   ORDER by customer_po  ";
		...
		else
		{
			...
			$where = "  and A.type_of_bill='Tax Invoice' and B.from_date between '$search_date1' and '$search_date2' GROUP BY A.schedule_id   ORDER by customer_po ASC  ";
			$result['res2'] = $this->Dispatchmodel->schedule_serach_query($where);
